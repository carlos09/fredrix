...
add_filter( 'wpcf7_mail_components', 'pdf_product_attachments' );
	function pdf_product_attachments( $components ) { 
	$pdf = array();
	
	foreach($_POST['interestedin'] as $selected){
		$selected = 'wp-content/uploads/product-pdfs/' . str_replace(' ', '', $selected) . '.pdf';	
		$pdf[] = $selected;
	}

	$components['attachments'] = $pdf; 
